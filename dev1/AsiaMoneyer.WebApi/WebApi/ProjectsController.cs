...
namespace AsiaMoneyer.WebApi.Controllers
{
    ...
    public class ProjectsController : BaseApiController
    {
        ...
        [HttpPost]
        public IHttpActionResult GetFavoriteProjectHeaders()
        {
            this._projectAppService.UserId = User.Identity.GetUserId();
            var projectDto = this._projectAppService.GetFavoriteProjectHeaders(this._projectAppService.UserId);
            return Ok(projectDto);
        }

        [HttpPost]
        public IHttpActionResult GetProjectSummary(ProjectDto pj)
        {
            var projectDto = this._projectAppService.GetProjectSummary(pj.Id);
            return Ok(projectDto);
        }

        [HttpPost]
        public IHttpActionResult GetProjects(Project.Dtos.ProjectFilterDto filter)
        {
            var user = AppUserManager.FindByNameAsync(User.Identity.Name).Result;
            if (user != null)
            {
                filter.UserId = user.Id;
                var l = this._projectAppService.GetProjects(filter);
                return Ok(l);
            }
        ...
        [HttpPost]
        public IHttpActionResult SetProjectArchived(Project.Dtos.ProjectDto projectDto)
        {
            try
            {
                this._projectAppService.UserId = User.Identity.GetUserId();
                this._projectAppService.SetProjectArchived(projectDto);
                return Ok();
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public IHttpActionResult SetProjectFavorite(Project.Dtos.ProjectDto projectDto)
        {
            try
            {
                this._projectAppService.UserId = User.Identity.GetUserId();
                this._projectAppService.SetProjectFavorite(projectDto);
                return Ok();
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public IHttpActionResult GetProjectRecentActivity(Project.Dtos.PagingDto pagingDto)
        {
            var l = this._projectAppService.GetProjectRecentActivity(pagingDto.Id,pagingDto);
            return Ok(l);
        }
        ...
        [HttpPost]
        public IHttpActionResult GetProjectCategories(CategoryDto categoryDto)
        {
            try
            {
                var l = this._categoryAppService.GetProjectCategories(categoryDto.ProjectId);
                return Ok(l);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public bool CheckParent(CategoryDto categoryDto)
        {
            bool isParent = this._categoryAppService.CheckParent(categoryDto);
            return isParent;
        }
        ...
        [HttpPost]
        public IHttpActionResult GetParentCategories(CategoryDto categoryDto)
        {
            bool Income = categoryDto.IsIncome??false;
            var l = this._categoryAppService.GetRootCategories(categoryDto.ProjectId, categoryDto.Id,Income);
            return Ok(l);
        }

        public IHttpActionResult SaveParentCategory(CategoryDto categoryDto)
        {
            this._categoryAppService.SaveParentCategory(categoryDto);
            return Ok();
        }
        #endregion
        ...
        [HttpPost]
        public IHttpActionResult TransferMoney(Project.Dtos.TransferMoneyDto transferMoneyDto)
        {
            try
            {
                this._transactionAppService.UserId = User.Identity.GetUserId();
                this._transactionAppService.TransferMoney(transferMoneyDto);
                return Ok();
            }
            catch (Exception ex)
            {
