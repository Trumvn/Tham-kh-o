...
namespace AsiaMoneyer.Security.Infrastructure
{
    public class ApplicationGroupManager
    {
        ...
        public ApplicationGroupManager(IOwinContext context)
        {
            ...
            this._db = context.Get<ApplicationDbContext>(); ;
            this._userManager = context.GetUserManager<ApplicationUserManager>(); ;
            this._roleManager = context.Get<ApplicationRoleManager>(); ;
            this._groupStore = new ApplicationGroupStore(_db);
        ...
        public IdentityResult SetGroupRoles(string groupId, params string[] roleNames)
        {
            ...
            {
                thisGroup.ApplicationRoles.Add(new ApplicationGroupRole { GroupId = groupId, RoleId = role.Id });
            }
            _db.SaveChanges();
        ...
        public IdentityResult SetUserGroups(string userId, params string[] groupNames)
        {
            var groupIds = this.Groups.Where(x => groupNames.Contains(x.Name)).Select(x => x.Id).ToArray();
            return this.SetUserGroupsById(userId, groupIds);
        }

        public IdentityResult SetUserGroupsById(string userId, params string[] groupIds)
        {
            // Clear current group membership:
        ...
        public async Task<IEnumerable<ApplicationUser>> GetGroupUsersAsync(string groupId)
        {
            ...
            {
                var user = _db.Users
                    .FirstOrDefault(u => u.Id == groupUser.UserId);
                users.Add(user);
            }
            return users;
        }
